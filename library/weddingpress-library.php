...
class WDP_Templates_Library extends Source_Base{
	...
	public static function get_template_content($template_id) {
		...
		if ( !$license ) {
			return new \WP_Error( 'license_missing', 'Your license is missing' );
		}

		$license_status = get_option( 'weddingpress_license_status', false );
		if ( ! ( isset( $license_status->license ) && $license_status->license == 'valid' ) ) {
			return new \WP_Error( 'license_invalid', 'Your license is not valid' );
		}

		if ( empty( $license ) ) {
			return new \WP_Error( 'no_license', __( 'License is not active.', 'weddingpress' ) );
		}
